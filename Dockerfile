...
RUN curl -sL -O https://github.com/BhallaLab/ReleaseScripts/archive/master.tar.gz 
RUN ls -la *.gz
RUN tar xvf master.tar.gz
RUN cd ReleaseScripts-master && ./build_wheels_linux.sh 
RUN cd ReleaseScripts-master && ./test_and_upload.sh $PYPI_PASSWORD
